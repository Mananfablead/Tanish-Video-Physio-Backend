...
const getAllSessions = async (req, res, next) => {
    try {
        ...
        res.status(200).json(ApiResponse.success({ sessions }, 'All sessions retrieved successfully'));
    } catch (error) {
        next(error);
    }
};
...
const getAllUpcomingSessions = async (req, res, next) => {
    try {
        ...
        res.status(200).json(ApiResponse.success({ sessions }, 'All upcoming sessions retrieved successfully'));
    } catch (error) {
        next(error);
    }
};
...
const createSession = async (req, res, next) => {
    try {
        const { bookingId, subscriptionId, date, time, type, status, duration } = req.body;

        // Validate that either bookingId or subscriptionId is provided
        if (!bookingId && !subscriptionId) {
            return res.status(400).json(ApiResponse.error('Either bookingId or subscriptionId must be provided'));
        }
        ...
        // Handle subscription-based session
            ...
            if (subscription.status !== 'active') {
                return res.status(400).json(ApiResponse.error('Cannot create session: Subscription is not active'));
            }

            // Check subscription validity dates
            const now = new Date();
            if (subscription.endDate && subscription.endDate < now) {
                return res.status(400).json(ApiResponse.error('Cannot create session: Subscription has expired'));
            }

            // Check session limits if the subscription plan has a limit
            const plan = await SubscriptionPlan.findById(subscription.planId);
            if (plan && plan.sessions > 0) { // 0 means unlimited sessions
                ...
                if (completedSessionsCount >= plan.sessions) {
                    return res.status(400).json(ApiResponse.error(`Cannot create session: Maximum session limit of ${plan.sessions} reached for this subscription`));
                }
            }
            ...
            if (!therapistId) {
                return res.status(400).json(ApiResponse.error('Therapist ID is required for subscription-based session'));
            }
...
const createAdminSession = async (req, res, next) => {
    try {
        ...
        const { bookingId, subscriptionId, userId, therapistId, date, time, type, status, duration, notes } = req.body;
        ...
        if (!userId || !therapistId) {
            return res.status(400).json(ApiResponse.error('User ID and Therapist ID are required for admin-created sessions'));
        }
...
const updateAdminSession = async (req, res, next) => {
    try {
        ...
        res.status(200).json(ApiResponse.success({ session: updatedSession }, 'Session updated successfully'));
    } catch (error) {
        next(error);
    }
};
